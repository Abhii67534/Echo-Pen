...
export const Blog = () => {
    ...
    const fetchBlogs = async () => {
      try {
        const response = await axios.get(
          "https://backend.abhisharma4950.workers.dev/post/bulk",
          {
            headers: {
        ...
        if (response.status === 200) {
          ...
          const authors = response.data.map((post: BlogObject) =>
  ...
  return (
    <div className="bg-rose-50 min-h-screen w-screen overflow-x-hidden">
        ...
        <nav className="flex justify-center">
          <div className="container flex justify-between items-center">
            ...
            <div className="flex items-center">
              <Button
                className="rounded-full ml-10 xs:w-[70px] xs:h-[30px] tab:w-[90px] tab:h-[35px]"
                onClick={handleWrite}
              >
                <img
                  className="tab:w-[15px] tab:h-[15px] hidden tab:block mr-2"
                  src="/images/pen.png"
                  alt="Write Icon"
                />
                Write
              ...
              <Button
                className="rounded-full tab:ml-10 xs:ml-4 xs:w-[70px] xs:h-[30px] tab:w-[80px] tab:h-[38px]"
      ...
      <div className="flex flex-col md:flex-row min-h-screen">
        ...
        {/* This is the new vertical line */}
        {/* RIGHT SIDE */}
        <div className="w-full md:w-1/4 min-h-screen pl-10 pt-10 hidden lg:flex flex-col items-center mr-10">
          ...
          <div className="mt-10 ml-2">
            <div className="font-bold mb-5">Random Picks</div>
            <div className="flex flex-col">
              {randomAuthors.map((author) => (
                <Button
                  className="w-[150px] mb-4"
                  key={author}
                  onClick={() => displayRandom(author)}
                >
                  {author}
                </Button>
              ))}
          ...
          <div className="w-[300px] h-[200px] rounded-lg bg-red-200 mt-10">
            <div className="font-bold flex justify-center mb-10 pt-5">
              Not sure what to write?
            </div>
            <div className="flex justify-center mb-3 font-medium">
              <p>Ask&nbsp; </p> <p className="italic">Jimmy</p>
            </div>
            <div className="flex justify-center font-medium">
              <Button className="rounded-full" onClick={handleWrite}>
                Go To Jimmy
